...
with torch.no_grad():
    roberta_model = EmoClassificationModel(AutoModelWithLMHead.from_pretrained("roberta-base").base_model, len(labels))
    roberta_model.load_state_dict(torch.load('RoBERTa_emotion_best.pt', map_location=torch.device('cpu'))) #change path

#load emotion classifier (T5)
#with torch.no_grad():
#    emomodel = T5FineTuner(args)
#    emomodel.load_state_dict(torch.load('T5emotion_finetuned.pt', map_location=torch.device('cpu')))

#load empathy classifier (T5)
with torch.no_grad():
    t5model = T5FineTuner(args)
    t5model.load_state_dict(torch.load('T5_empathy_best.pt', map_location=torch.device('cpu'))) #change path
...
def get_emotion(text):
  ...
  with torch.no_grad():
      output = roberta_model((sequence_padded, attention_mask_padded))
